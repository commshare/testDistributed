...
type Master struct {
	...
	address         string
	registerChannel chan string /*这个channel里是告诉matster 那些worker处于idle状态需要新的work的*/
	...
	nReduce int      // Number of reduce partitions 分区
...
/*这个是串行执行的，先map再reduce，等待每个task完成，然后再调度下一个*/
...
func Sequential(jobName string, files []string, nreduce int,
	...
	mr = newMaster("master")
	go mr.run(jobName, files, nreduce, func(phase jobPhase)/*调度函数*/ {
		switch phase {
		case mapPhase:
...
/*通过rpc来分布式的调度这些worker执行mapreduce的task*/
...
// Wait blocks until the currently scheduled work has completed. 阻塞直到当前的调度任务完成
...
/*这个功能可以用来做为紧急停止操作，master发shutdown给agent停止*/
